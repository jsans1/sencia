...
import svgPaths from "../imports/svg-jc9m1p7sb6";
  ...
  return (
    <div className="absolute box-border content-stretch flex flex-col gap-2 items-center justify-center left-0 p-[16px] top-[756px] w-[393px]">
      <button 
        onClick={onClick}
        disabled={disabled}
        className={`h-[52px] relative rounded-[20px] shrink-0 w-full transition-opacity ${
          disabled ? 'bg-[#d9d9d9] cursor-not-allowed' : 'bg-[#212121] hover:opacity-90 cursor-pointer'
        }`}
        ...
        <div className="flex flex-row items-center justify-center relative size-full">
          <div className="box-border content-stretch flex gap-2.5 h-[52px] items-center justify-center p-[20px] relative w-full">
            <div className={`font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-center text-nowrap ${
              disabled ? 'text-[#999999]' : 'text-white'
            }`}>
    ...
    <div className="content-stretch flex flex-col gap-2 items-start justify-start relative shrink-0 w-full">
      <div className="font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[26px] text-black w-full">
        <p className="leading-[1.1]">
          ...
          <span>{` rendez-vous avec votre praticien`}</span>
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="content-stretch flex flex-col gap-2.5 items-start justify-start relative shrink-0 w-[333px]">
      ...
      <div className="font-['SF_Pro_Display:Regular',_sans-serif] h-[34px] leading-[0] not-italic relative shrink-0 text-[#7a7a7a] text-[14px] w-[333px]">
        <p className="leading-[normal]">Entrez le spécialiste que vous avez consulté et la date de ce dernier rendez-vous.</p>
      </div>
    </div>
  );
}

function DropdownItem({ selectedDoctor }: { selectedDoctor: string }) {
  return (
    <div className="content-stretch flex flex-col gap-1 items-start justify-center leading-[0] not-italic relative shrink-0 text-nowrap" data-name="Dropdown Item">
      <div className="font-['SF_Pro_Display:Medium',_sans-serif] relative shrink-0 text-[14px] text-[rgba(0,0,0,0.4)]">
        <p className="leading-[normal] text-nowrap whitespace-pre">Typologie de praticien</p>
      </div>
      <div className="font-['SF_Pro_Display:Regular',_sans-serif] relative shrink-0 text-[18px] text-black">
        <p className="leading-[normal] text-nowrap whitespace-pre">{selectedDoctor}</p>
      </div>
    </div>
  );
}

function DropdownContainer({ selectedDoctor, onDoctorChange }: { selectedDoctor: string; onDoctorChange: (doctor: string) => void }) {
  const [isOpen, setIsOpen] = useState(false);
  const doctors = ["Cardiologue", "Médecin généraliste", "Pneumologue", "Neurologue", "Dermatologue"];

  return (
    <div className="relative">
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="bg-white h-[70px] relative rounded-[16px] shrink-0 w-full cursor-pointer"
      ...
      >
        ...
        <div className="flex flex-row items-center relative size-full">
          <div className="box-border content-stretch flex h-[70px] items-center justify-between p-[16px] relative w-full">
            <DropdownItem selectedDoctor={selectedDoctor} />
            <div className="h-1.5 relative shrink-0 w-3" data-name="Icon">
              <div className="absolute inset-[-16.67%_-8.33%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 8">
                  <path d="M1 1L7 7L13 1" id="Icon" opacity="0.78" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" strokeOpacity="0.11" strokeWidth="2" />
                </svg>
              </div>
            </div>
          </div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#d9d9d9] rounded-[16px] shadow-lg z-50 max-h-48 overflow-y-auto">
          {doctors.map((doctor, index) => (
            <button
              ...
              className="w-full text-left px-4 py-3 hover:bg-gray-50 first:rounded-t-[16px] last:rounded-b-[16px] border-b border-gray-100 last:border-b-0"
            >
...
function Frame6202({ selectedDoctor, onDoctorChange }: { selectedDoctor: string; onDoctorChange: (doctor: string) => void }) {
  return (
    <div className="content-stretch flex flex-col gap-2.5 items-start justify-start relative shrink-0 w-full">
      <div className="font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-black w-full">
        <p className="leading-[1.2]">Votre médecin</p>
      </div>
      <DropdownContainer selectedDoctor={selectedDoctor} onDoctorChange={onDoctorChange} />
    </div>
  );
}
...
function DropdownContainer1({ day, month, year, onDateChange }: { 
  day: string; 
  month: string; 
  year: string; 
  onDateChange: (field: 'day' | 'month' | 'year', value: string) => void 
...
function Frame6201({ day, month, year, onDateChange }: { 
  ...
  return (
    <div className="content-stretch flex flex-col gap-2.5 items-start justify-start relative shrink-0 w-full">
      <div className="font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-black w-full">
        <p className="leading-[1.2]">Entrez la date de ce dernier rendez-vous</p>
      </div>
      <DropdownContainer1 day={day} month={month} year={year} onDateChange={onDateChange} />
    </div>
  );
}

function Container({ selectedDoctor, onDoctorChange, day, month, year, onDateChange }: {
  ...
  return (
    <div className="content-stretch flex flex-col gap-8 items-start justify-start relative shrink-0 w-full" data-name="Container">
      <Frame6202 selectedDoctor={selectedDoctor} onDoctorChange={onDoctorChange} />
      <Frame6201 day={day} month={month} year={year} onDateChange={onDateChange} />
    </div>
  );
}

function Container1({ selectedDoctor, onDoctorChange, day, month, year, onDateChange }: {
  selectedDoctor: string;
  onDoctorChange: (doctor: string) => void;
  day: string;
  month: string;
...
function Frame6200({ selectedDoctor, onDoctorChange, day, month, year, onDateChange }: {
  selectedDoctor: string;
  onDoctorChange: (doctor: string) => void;
  day: string;
  month: string;
...
export default function Export2Specialiste({ onBack, onClose, onContinue }: Export2SpecialisteProps) {
