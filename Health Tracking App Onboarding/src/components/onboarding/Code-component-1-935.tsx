...
export function ChronicIllnessStep({ data, onUpdate, onNext, onBack, onClose }: ChronicIllnessStepProps) {
  ...
  const isValid = data.condition && data.diagnosisDate.day && data.diagnosisDate.month && data.diagnosisDate.year;
  ...
  return (
    <div className="bg-white overflow-clip relative rounded-[44px] size-full" data-name="infos perso">
      ...
      {/* Continue Button */}
      <div className="absolute bg-[#212121] box-border content-stretch flex gap-2.5 items-center justify-center left-4 px-5 py-4 rounded-[20px] top-[756px] w-[361px]">
        ...
        <Button 
          className="font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic text-[18px] text-center text-nowrap text-white bg-transparent border-0 p-0 hover:bg-transparent disabled:bg-transparent disabled:opacity-50"
          onClick={onNext}
          disabled={!isValid}
        >
          Continuer
