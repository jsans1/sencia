...
function StressSlider({ value, onChange }: StressSliderProps) {
  ...
  const updateValue = useCallback((clientX: number) => {
    ...
    // Map horizontal position to stress value (left = 0% stress, right = 100% stress)
  ...
  const handlePosition = (value / 100) * sliderWidth;

  return (
    <div className="flex h-[38px] items-center justify-center w-[327px] relative">
      <div 
        ref={sliderRef}
      ...
      <div 
        ...
        style={{ left: `${Math.max(16, Math.min(sliderWidth - 16, handlePosition))}px` }}
        onMouseDown={handleMouseDown}
        onTouchStart={handleTouchStart}
      >
...
export function StressLevelScreen({ initialValue, onBack, onContinue }: StressLevelScreenProps) {
  ...
  return (
    <div className="bg-white overflow-clip relative rounded-[44px] size-full" data-name="Logging1-ressenti">
      ...
      <div className="absolute left-[33px] top-[380px]">
        <StressSlider value={stressValue} onChange={setStressValue} />
      </div>
      
      <div className="absolute font-['SF_Pro_Display:Regular',_sans-serif] leading-[0] left-[50px] not-italic text-[14px] text-black text-center text-nowrap top-[345px] translate-x-[-50%]">
        <p className="leading-[normal] whitespace-pre">Aucun stress</p>
      </div>
      
      <div className="absolute font-['SF_Pro_Display:Regular',_sans-serif] leading-[0] left-[343px] not-italic text-[14px] text-black text-center text-nowrap top-[345px] translate-x-[-50%]">
