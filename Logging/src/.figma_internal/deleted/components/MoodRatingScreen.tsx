...
function MoodSlider({ value, onChange }: MoodSliderProps) {
  ...
  // Calculate slider handle position (invert the value since top = 100%)
  const sliderHeight = 327;
  const handlePosition = ((100 - value) / 100) * sliderHeight;

  return (
    <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
      {/* "Très bien" label */}
      <div className="absolute left-1/2 transform -translate-x-1/2 -top-[20px] font-['SF_Pro_Display:Regular',_sans-serif] text-[14px] text-black text-center">
        <p className="text-[14px] text-nowrap">Très bien</p>
      </div>
      ...
      <div className="relative">
        <div 
          ref={sliderRef}
          className="w-[38px] h-[327px] cursor-pointer flex items-center justify-center"
          onMouseDown={handleMouseDown}
          onTouchStart={handleTouchStart}
        >
          {/* Vertical gradient bar */}
          <div className="w-[38px] h-full bg-gradient-to-b from-[#62ffa4] via-[#ffb48b] to-[#ff5d5d] rounded-[19px]" />
        </div>
        
        {/* Slider handle */}
        <div 
          className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 cursor-pointer"
      ...
      {/* "Très mal" label */}
      <div className="absolute left-1/2 transform -translate-x-1/2 -bottom-[20px] font-['SF_Pro_Display:Regular',_sans-serif] text-[14px] text-black text-center">
        <p className="text-[14px] text-nowrap">Très mal</p>
      </div>
    </div>
  );
}
          ...
          <div className="flex flex-row items-center justify-center relative size-full">
            <div className="box-border content-stretch flex gap-2.5 h-[52px] items-center justify-center p-[20px] relative w-full">
              <div className={`font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-center text-nowrap ${
                hasInteracted ? 'text-white' : 'text-[#a0a0a0]'
              }`}>
