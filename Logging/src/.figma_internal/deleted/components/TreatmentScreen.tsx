...
import svgPaths from "../imports/svg-ik0sl021dz";

interface TreatmentScreenProps {
  initialValue: boolean | null; // null = middle, true = yes, false = no
  onBack: () => void;
  onContinue: (value: boolean | null) => void;
}
  ...
  return (
    <div className="content-stretch flex flex-col gap-2 items-start justify-start leading-[0] not-italic relative shrink-0 w-full">
      <div className="font-['SF_Pro_Display:Medium',_sans-serif] relative shrink-0 text-[26px] text-black w-full">
        <p className="leading-[1.1]">Avez-vous pris votre traitement aujourd'hui ?</p>
      </div>
      <div className="font-['SF_Pro_Display:Regular',_sans-serif] relative shrink-0 text-[#646464] text-[14px] w-full">
        <p className="leading-[normal]">Sélectionnez les traitements que vous avez pris jusqu'à présent. Fiez-vous toujours à votre ordonnance.</p>
      </div>
    </div>
  );
}
...
interface InteractiveDialProps {
  value: boolean | null; // null = middle, true = yes (right), false = no (left)
  onChange: (value: boolean | null) => void;
}

function InteractiveDial({ value, onChange }: InteractiveDialProps) {
  ...
  const getAngleFromPosition = useCallback((clientX: number, clientY: number) => {
    ...
    // Calculate angle in degrees, starting from top (0°) and going clockwise
  ...
  const getValueFromAngle = useCallback((angle: number) => {
    // Map angles to values:
    // 270° (left) = false (no)
    // 0° (top) = null (middle)  
    // 90° (right) = true (yes)
    
    if (angle >= 315 || angle <= 45) {
      return null; // Middle position
    } else if (angle > 45 && angle <= 135) {
      return true; // Right position (yes)
    } else {
      return false; // Left position (no)
    }
  }, []);

  const getAngleFromValue = useCallback((val: boolean | null) => {
    if (val === null) return 0; // Top (middle)
    if (val === true) return 90; // Right (yes)
    return 270; // Left (no)
  }, []);
  ...
  const updateValue = useCallback((clientX: number, clientY: number) => {
    ...
    onChange(newValue);
  }, [getAngleFromPosition, getValueFromAngle, onChange]);
  ...
  const handleX = 172 + 120 * Math.sin(currentAngle * Math.PI / 180);
  const handleY = 172 - 120 * Math.cos(currentAngle * Math.PI / 180);
  ...
  return (
    <div className="grid-cols-[max-content] grid-rows-[max-content] inline-grid leading-[0] place-items-start relative shrink-0" data-name="Roue">
      <div 
        ref={dialRef}
        className="[grid-area:1_/_1] ml-0 mt-0 relative size-[340px] cursor-pointer" 
        data-name="Vector"
        ...
        <div className="absolute inset-[-0.441%]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 344 344">
            <g id="Vector">
              ...
              <path d={svgPaths.p3e9a9300} stroke="var(--stroke-0, #DADADA)" strokeWidth="3" />
            </g>
          </svg>
        </div>
      </div>
      ...
      {/* Labels */}
      <div className="[grid-area:1_/_1] box-border content-stretch flex font-['SF_Pro_Display:Semibold',_sans-serif] h-[15px] items-center justify-between ml-16 mt-[162px] not-italic relative text-[16px] text-black text-center text-nowrap w-[213px]">
        <div className="relative shrink-0">
          <p className="leading-[normal] text-nowrap whitespace-pre">Oui</p>
        </div>
        <div className="relative shrink-0">
          <p className="leading-[normal] text-nowrap whitespace-pre">Non</p>
        </div>
      </div>
      
      {/* Interactive handle */}
      <div 
        className="absolute size-[43px] cursor-pointer transition-all duration-200"
        style={{ 
          left: `${handleX - 21.5}px`, 
          top: `${handleY - 21.5}px`,
        }}
        onMouseDown={handleMouseDown}
        onTouchStart={handleTouchStart}
      >
        <div className="absolute inset-[-45.12%_-35.81%_-26.51%_-35.81%]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 75 75">
            <g filter="url(#filter0_d_1_626)" id="Ellipse 1487">
              <circle cx="37.5" cy="41.5" fill="var(--fill-0, white)" r="21.5" />
              <circle cx="37.5" cy="41.5" r="21" stroke="var(--stroke-0, #DADADA)" />
            </g>
            <defs>
              <filter colorInterpolationFilters="sRGB" filterUnits="userSpaceOnUse" height="73.8" id="filter0_d_1_626" width="73.8" x="0.6" y="0.6">
                <feFlood floodOpacity="0" result="BackgroundImageFix" />
                ...
                <feBlend in2="BackgroundImageFix" mode="normal" result="effect1_dropShadow_1_626" />
                <feBlend in="SourceGraphic" in2="effect1_dropShadow_1_626" mode="normal" result="shape" />
              </filter>
            </defs>
          </svg>
        </div>
...
export function TreatmentScreen({ initialValue, onBack, onContinue }: TreatmentScreenProps) {
  const [treatmentValue, setTreatmentValue] = useState<boolean | null>(initialValue);

  const hasSelection = treatmentValue !== null;

  const handleContinue = () => {
    if (hasSelection) {
      onContinue(treatmentValue);
    }
  };

  return (
    <div className="bg-white overflow-clip relative rounded-[44px] size-full" data-name="Logging4-traitement">
      <BgLinear />
      <HomeBar />
      ...
      <div className="absolute content-stretch flex flex-col items-start justify-start left-0 top-0 w-[393px]">
        ...
        <div className="box-border content-stretch flex flex-col gap-[50px] h-[533px] items-center justify-center px-[30px] py-10 relative shrink-0 w-[393px]">
          <Header />
          <InteractiveDial value={treatmentValue} onChange={setTreatmentValue} />
        </div>
      </div>
          ...
          <div className="flex flex-row items-center justify-center relative size-full">
            <div className="box-border content-stretch flex gap-2.5 h-[52px] items-center justify-center p-[20px] relative w-full">
              <div className={`font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-center text-nowrap ${
                hasSelection ? 'text-white' : 'text-[#a0a0a0]'
              }`}>
