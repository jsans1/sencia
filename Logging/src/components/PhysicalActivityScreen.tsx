...
function ActivityDial({ value, onChange }: ActivityDialProps) {
  ...
  const getValueFromAngle = useCallback((angle: number) => {
    // Map angles to values with larger zones for easier interaction:
    // 30° - 150° (right): yes_30_plus  
    // 210° - 330° (left): yes_30_minus  
    // 150° - 210° (down): no
  ...
  const getAngleFromValue = useCallback((val: 'yes_30_plus' | 'yes_30_minus' | 'no' | null) => {
    if (val === 'yes_30_plus') return 90; // Right
    if (val === 'yes_30_minus') return 270; // Left
    if (val === 'no') return 180; // Down
    return 90; // Default to right
  }, []);
  ...
  return (
    <div className="grid-cols-[max-content] grid-rows-[max-content] inline-grid leading-[0] place-items-start relative shrink-0" data-name="Roue">
      ...
      {/* Labels */}
      <div className="[grid-area:1_/_1] font-['SF_Pro_Display:Semibold',_sans-serif] ml-[253px] mt-[100px] not-italic relative text-[16px] text-black text-center translate-x-[-50%] w-[92px]">
      ...
      <div className="[grid-area:1_/_1] font-['SF_Pro_Display:Semibold',_sans-serif] ml-[87px] mt-[100px] not-italic relative text-[16px] text-black text-center translate-x-[-50%] w-[92px]">
        <p className="leading-[normal]">
          Oui,
          <br aria-hidden="true" />
          - de 30 mins
      ...
      >
        <div className="relative w-full h-full">
          <div className="absolute inset-0 bg-white rounded-full border border-[#DADADA] shadow-lg" 
               style={{ 
...
export function PhysicalActivityScreen({ initialValue, onBack, onContinue }: PhysicalActivityScreenProps) {
  ...
  const hasSelection = activityValue !== null;
