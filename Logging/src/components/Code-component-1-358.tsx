...
import svgPaths from "../imports/svg-0ar3p2692j";

interface MoodRatingScreenProps {
  initialValue: number;
      ...
      <div className="flex flex-col items-center justify-center relative size-full">
        <div className="box-border content-stretch flex flex-col gap-3 items-center justify-center leading-[0] not-italic px-[30px] py-10 relative w-full">
          <div className="font-['SF_Pro_Display:Medium',_sans-serif] relative shrink-0 text-[26px] text-black w-full">
            <p className="leading-[1.1]">Comment vous sentez-vous ce matin Alima ?</p>
          </div>
...
function MoodSlider({ value, onChange }: MoodSliderProps) {
  ...
  const sliderPosition = ((100 - value) / 100) * 327; // 327px is the slider height

  return (
    <div className="relative">
      {/* "Très bien" label */}
      <div className="absolute font-['SF_Pro_Display:Regular',_sans-serif] leading-[0] left-[33px] not-italic text-[14px] text-black text-center text-nowrap top-[14px] translate-x-[-50%]">
        <p className="leading-[normal] whitespace-pre">Très bien</p>
      </div>
      
      {/* Slider container */}
      <div className="absolute flex h-[327px] items-center justify-center left-[14px] top-[48px] w-[38px]">
        <div 
          ref={sliderRef}
          className="flex-none rotate-[90deg] cursor-pointer"
          onMouseDown={handleMouseDown}
          onTouchStart={handleMouseDown}
        >
          <div className="bg-gradient-to-r from-[#62ffa4] h-[38px] rounded-[80px] to-[#ff5d5d] via-[#ffb48b] via-[51.923%] w-[327px]" />
        </div>
      </div>
      
      {/* Slider handle */}
      <div 
        className="absolute flex h-[32px] items-center justify-center left-[17px] w-[32px] cursor-pointer transition-all duration-200"
        style={{ top: `${64 + sliderPosition}px` }}
        onMouseDown={handleMouseDown}
        onTouchStart={handleMouseDown}
      >
      ...
      {/* "Très mal" label */}
      <div className="absolute font-['SF_Pro_Display:Regular',_sans-serif] leading-[0] left-[32px] not-italic text-[14px] text-black text-center text-nowrap top-[391px] translate-x-[-50%]">
        <p className="leading-[normal] whitespace-pre">Très mal</p>
      </div>
    </div>
  );
}

export function MoodRatingScreen({ initialValue, onContinue }: MoodRatingScreenProps) {
  const [moodValue, setMoodValue] = useState(initialValue);

  const handleContinue = () => {
    onContinue(moodValue);
  };
