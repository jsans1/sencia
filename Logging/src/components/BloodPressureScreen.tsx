                ...
                <div className="flex flex-row items-center overflow-clip relative size-full">
                  <div className="box-border content-stretch flex items-center justify-start px-4 py-2 relative w-full">
                    <input
                      ...
                      value={systolic}
                      onChange={(e) => {
                        // Allow only numbers, spaces, and common medical units
                        const value = e.target.value.replace(/[^0-9\s\/\-\.\,mmHg]/g, '');
                        onSystolicChange(value);
                      }}
                      onKeyDown={(e) => {
                        // Allow numbers, backspace, delete, arrow keys, tab, and some special chars
                        const allowedKeys = ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Tab', 'Enter'];
                        const allowedChars = '0123456789 /-.,';
                        
                        if (!allowedKeys.includes(e.key) && !allowedChars.includes(e.key) && !e.ctrlKey && !e.metaKey) {
                          e.preventDefault();
                        }
                      }}
      ...
      <div className="relative size-full">
        <div className="box-border content-stretch flex flex-col gap-[13px] items-start justify-start p-[20px] relative w-full">
          <div className="content-stretch flex gap-3 items-start justify-start relative shrink-0 w-full">
            <div className="relative shrink-0 size-[18px] mt-1" data-name="file-text">
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 18">
                <g id="file-text">
            ...
            <textarea
              ...
              placeholder="Notes..."
              className="flex-1 bg-transparent font-['SF_Pro_Display:Regular',_sans-serif] text-[16px] text-black placeholder-[#a0a0a0] outline-none resize-none min-h-[100px] max-h-[150px] leading-[1.4]"
              rows={5}
            />
          </div>
        </div>
...
export function BloodPressureScreen({ initialData, onBack, onContinue }: BloodPressureScreenProps) {
  ...
  const hasRequiredData = systolic.trim() !== '' && diastolic.trim() !== '';
          ...
          <div className="flex flex-row items-center justify-center relative size-full">
            <div className="box-border content-stretch flex gap-2.5 h-[52px] items-center justify-center p-[20px] relative w-full">
              <div className={`font-['SF_Pro_Display:Medium',_sans-serif] leading-[0] not-italic relative shrink-0 text-[16px] text-center text-nowrap ${
                hasRequiredData ? 'text-white' : 'text-[#a0a0a0]'
              }`}>
