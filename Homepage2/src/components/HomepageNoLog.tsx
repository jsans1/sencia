import svgPaths from "../imports/svg-vs8whw9wr6";

interface HomepageNoLogProps {
  onPlusClick: () => void;
  onBackClick: () => void;
}
...
function Frame6101() {
  return (
    <button
      className="bg-[#292929] h-[67.31px] relative rounded-[312.974px] shrink-0 w-full transition-transform hover:scale-105 active:scale-95"
      onClick={() => {}} // We'll handle this at parent level
    >
      <div className="flex flex-row items-center justify-center relative size-full">
...
function Frame6187({ onPlusClick }: { onPlusClick: () => void }) {
  return (
...
function Frame6186({ onPlusClick }: { onPlusClick: () => void }) {
  return (
    <div className="content-stretch flex items-start justify-start relative shrink-0">
      ...
      <Frame6187 onPlusClick={onPlusClick} />
    </div>
  );
}
...
function Frame6190({ onPlusClick }: { onPlusClick: () => void }) {
  return (
    <div className="absolute content-stretch flex flex-col gap-10 items-center justify-start left-1/2 translate-x-[-50%] translate-y-[-50%] w-[327px]" style={{ top: "calc(50% - 11.5px)" }}>
      <Frame6150 />
      <Frame6186 onPlusClick={onPlusClick} />
      <Frame6076 />
    </div>
  );
}
...
function Frame6106({ onPlusClick }: { onPlusClick: () => void }) {
  return (
    <button
      onClick={onPlusClick}
...
function Nav({ onPlusClick }: { onPlusClick: () => void }) {
  return (
    <div className="absolute bg-[#faf9f9] bottom-0 h-[120px] left-1/2 translate-x-[-50%] w-[393px]" data-name="nav">
      <div className="box-border content-stretch flex h-[120px] items-start justify-between overflow-clip pb-[26px] pt-[15px] px-7 relative w-[393px]">
        <Frame6107 onPlusClick={onPlusClick} />
        <HomeBar />
      </div>
...
export default function HomepageNoLog({ onPlusClick, onBackClick }: HomepageNoLogProps) {
  return (
    <div className="bg-white block overflow-clip relative rounded-[44px] size-full" data-name="Homepage_no log">
      <BgLinear />
      <ProgressBar />
      ...
      <Frame6190 onPlusClick={onPlusClick} />
      <Nav onPlusClick={onPlusClick} />
      
      {/* Back button */}
      <button 
        onClick={onBackClick}
        className="absolute left-5 top-20 p-2 rounded-full hover:bg-gray-100 transition-colors z-10"
      >
